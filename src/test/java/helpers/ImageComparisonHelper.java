...
public class ImageComparisonHelper {
    public static void compare(String fileName) {
        try {File baselineFile = new File(".gauge/screenshots/visual/baseline/" + fileName + ".png");
        ...
        } catch(Exception e){throw new RuntimeException("Image comparison failed : " + e.getMessage());
        }
    }
    private static boolean compareImages(BufferedImage img1, BufferedImage img2
        ...
        for(int x=0; x<img1.getWidth(); x++){for(int y=0; y<img1.getHeight(); y++){if(img1.getRGB(x,y) != img2.getRGB(x,y)){return false;}}
        }
        return true;
    }
